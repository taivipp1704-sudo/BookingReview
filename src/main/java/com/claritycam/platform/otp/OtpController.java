...
public class OtpController {
  ...
  public OtpController(OtpService otpService, CustomerAccountService customerAccounts) {
  ...
  OtpService.RequestedOtp request(@Valid @RequestBody OtpRequest request, HttpServletRequest servletRequest) {
    ...
    return otpService.request(request.phone(), request.purpose(), servletRequest.getRemoteAddr());
  }

  @PostMapping("/verify")
  OtpService.VerifiedOtp verify(@Valid @RequestBody VerifyOtpRequest request, HttpServletRequest servletRequest) {
    requireBookingSession(request.phone(), request.purpose(), servletRequest);
    return otpService.verify(request.challengeId(), request.phone(), request.code(), request.purpose(), servletRequest.getRemoteAddr());
  }
